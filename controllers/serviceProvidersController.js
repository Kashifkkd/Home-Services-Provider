...
const signup_get = (req,res) => {
    res.render('business/business-signup.ejs');
}

const login_get = (req,res) => {
    res.render('business/business-login.ejs');
}
...
const signup_post = async (req,res) => {
    ...
    try{
        const business = await Business.create({ username, email, password, confirmPassword});
...
const dashboard_post = async (req, res) => {
    ...
    const { email, shopName, firstName, lastName, mobileNumber } = req.body;
    console.log(email, shopName, firstName, lastName, mobileNumber);

    try{
        const businessUser = await Business.findOne({ email })
        const updateBusinessData = { shopName, firstName, lastName, mobileNumber};
        await businessUser.updateOne(updateBusinessData);
        const updatedUser = await businessUser.findById(user._id);
...
const dashboard = async (req, res) => {
    ...
    try{
        ...
        if(email){
            ...
            const mNumber = user.mobileNumber;
            const shopName = user.shopName;

            res.render('business/business-dashboard', { email , username, fName, lName, mNumber, shopName });
        } else {
